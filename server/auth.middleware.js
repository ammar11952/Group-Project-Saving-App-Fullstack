...
const requireAuth = (req, res, next) => {
  ...
  try {
    ...
    jwt.verify(token[1], process.env.SECRET_KEY, (err, decodedToken) => {
    ...
    if (!token || token[0] != 'Bearer') {
      res.status(403).send('Auth Failure 1: No Bearer or null' + err.message);
    } else {
      console.log(error);
      res.status(401).send('Auth Failure 2: Server Signature' + err.message);
    }
    return;
  }
